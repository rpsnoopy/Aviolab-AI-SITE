...
- **Current Version**: v1.3.01 - Audit di onestà completo della pagina `ip-legal.html` contro i prompt agent v.10.11: card capability/use-case/esempi/trust riscritte per allinearsi a ciò che gli agenti effettivamente codificano (rimossi: "probabilità quantitative di concedibilità", "ITAR/EAR automatici", "sideground IP", "market standard", "10 banche dati", riferimenti "Fortune 500"). Architettura sezione §6 onestizzata a "8 specialisti attivi + 1 in early access (revisore brevettuale) + 2 coordinatori" mantenendo big-number "9+2"; chip "Programma early access" anche sulla card cap4 Patent stress test. Nuovo esempio §5 ex9 (multilingua/cross-jurisdiction), sezione §8 self-validated patent riscritta con MCP/coding agentico. Eliminato il riferimento "Fortune 500" dal case study, sostituito con "grande gruppo industriale multinazionale". Lista banche dati brevettuali ridotta alle 7 effettivamente codificate nel prompt BREVETTI_DRAFTER (Espacenet, EPO Register, WIPO PATENTSCOPE, Google, Lens.org, UIBM, USPTO). Aggiunto cache buster su translations.js (`?v=131`). Aggiunta scheda AGID firmata in pa-locale.html (hero + nuova sezione "Documentazione e certificazioni" + voce in downloads.html), brochure PA aggiornata. Rimosso "Richieda una demo" dall'hero pa-locale, CTA finale solo con "Richieda una demo".
- **Previous Version**: v1.3.00 - Refactor architetturale di `ip-legal.html`: nuova pagina con 9 sezioni allineate alla realtà del prodotto, 4 capability card con bullet+proof, deliverable ricostruiti per casi d'uso reali, sezione "Built for production legal work" con L1-L5/no-hallucination/scoring/confidence. Vecchia ip-legal.html spostata a `ip-legal-v1-backup.html`. Eliminati riferimenti AEN.
