...
- **Current Version**: v1.2.04 - Hero CTA dei due prodotti ora affiancati side-by-side su desktop (flex-row con wrap, max-width 640px) anziché impilati verticalmente; collasso automatico in stack a tutta larghezza sotto i 600px. Dimensioni label/sub ricalibrate per il nuovo layout orizzontale.
- **Previous Version**: v1.2.03 - Hero buttons riprogettati su due righe (label prodotto bold + sottotesto descrittivo fra parentesi), bottoni TTR-SUITE per la PA Locale e TTR-SUITE IP Legal verticali al posto di "Explore/Contact". Allineamento generale del sito al registro formale "Lei + congiuntivi" (translations IT, privacy.html). Copy hero EN/IT aggiornato sul posizionamento "Agentic AI per due ambiti distinti, fonti sempre verificate e tracciabili". Cloudflare Web Analytics attivo via Automatic setup (no cookie banner richiesto).
