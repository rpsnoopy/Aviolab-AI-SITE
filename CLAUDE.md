...
- **Current Version**: v1.2.03 - Hero buttons riprogettati su due righe (label prodotto bold + sottotesto descrittivo fra parentesi), bottoni TTR-SUITE per la PA Locale e TTR-SUITE IP Legal verticali al posto di "Explore/Contact". Allineamento generale del sito al registro formale "Lei + congiuntivi" (translations IT, privacy.html). Copy hero EN/IT aggiornato sul posizionamento "Agentic AI per due ambiti distinti, fonti sempre verificate e tracciabili". Cloudflare Web Analytics attivo via Automatic setup (no cookie banner richiesto).
- **Previous Version**: v1.2.02 - Aggiunto modulo Assisted Drafting sulla landing IP Legal: stesura assistita di NDA, contratti di ricerca e accordi commerciali con draft validati internamente. Card IP dell'index aggiornata: "drafts and analyses" come nuovo posizionamento.
