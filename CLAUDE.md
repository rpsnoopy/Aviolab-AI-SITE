...
- **Current Version**: v1.3.09 - Nuova categoria **Certifications / Certificazioni** in `downloads.html` (id `#certificazioni`), fra Documentation e Client Area, con due sottogruppi: prima **PA Locale**, poi **Enterprise**, quattro documenti ciascuno (classificazione del rischio AI Act, informativa trasparenza, DPA Zero Data Retention, scheda tecnica). Sostituiscono l'**edizione v6.6** che stava dentro Documentation e che riportava la clausola ZDR sbagliata ("cancellazione entro 30 giorni", cioe' il comportamento SENZA accordo ZDR): i nuovi PDF sono il set **v7.0** firmato il 18 agosto 2026, allineato a TTR-SUITE F7.0.000. Otto file nuovi in `assets/downloads/` con prefisso di edizione (`ttr-suite-pa-*`, `ttr-suite-ent-*`), rimossi i quattro senza prefisso. **Corretto un disallineamento preesistente**: `ip-legal.html` linkava i PDF intestati "Edizione PA Locale" spacciandoli per Enterprise; ora punta al set Enterprise vero, e il sottotitolo passa da "Enterprise Edition v6.1" a "v7.0". `pa-locale.html` aggiornata al set PA v7.0 con le nuove dimensioni. Nuove chiavi `cert_*` + `downloads_cert_*` aggiunte per edit diretto a `js/translations.js` (EN/IT), rimosse le otto `download_classrisk/transparency/dpa/techspec` ormai orfane. Nuova classe `.downloads-subgroup` in `styles.css` per i titoli dei due gruppi. Cache buster translations.js a `?v=134`. Bump v1.3.09 nei footer delle 5 pagine.
