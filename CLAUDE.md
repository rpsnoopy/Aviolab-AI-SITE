...
- **Current Version**: v1.3.27 - **Brochure PA Locale riallineata** (`assets/downloads/ttr-suite-pa-brochure.pdf`, 1,09 MB, 4 pagine): sorgente `TTR-SUITE-ROOT/MARKET/02_MATERIALI_COMMERCIALI/TTR_SUITE_Brochure_Sindaci_4P_v3_2026-09.pptx` (derivata dalla v2_produzione del 2/7 con python-pptx, PDF esportato con PowerPoint via COM da PowerShell). Modifiche: «22 + 2» → «43» (24 agenti + 19 funzioni cognitive), paragrafo agenti riscritto, «Suite v.6.5» → «v.7.0», validazione con il Comune di Pieve Ligure nominato e la frase della Sindaca dal comunicato (tolto il riferimento all'annullamento in autotutela e alle 7 postazioni), «Polizia Municipale» → «Polizia Locale»; la dicitura AgID corretta c'era già nella v2. Layout di pagina 1 verificato a video dopo un primo giro con sbordamenti. Footer delle 7 pagine a v1.3.27.
- **Previous Version**: v1.3.26 - **`llms.txt` e `llms-full.txt` allineati al conteggio 43** (24 agenti + 19 funzioni cognitive): dicevano ancora «22+2 agenti» e «sedici agenti d'area», ed erano la fonte più probabile delle risposte vecchie date dagli assistenti AI con ricerca web (i file sono pensati proprio per loro). Aggiunto in llms.txt il riferimento a Pieve Ligure; «Polizia Municipale» → «Polizia Locale» in llms-full. Restano vecchi, e NON toccati: `assets/english_texts.txt` e `italian_texts.txt` (sorgenti non rigenerabili, chiave `home_card_pa_feat1` a «16+1») e la **brochure PDF** (`ttr-suite-pa-brochure.pdf`, dice 22: da rifare). Footer delle 7 pagine a v1.3.26.
- **Previous Version**: v1.3.25 - `dicono-di-noi.html`: aggiunto il **post della pagina Facebook del Comune di Pieve Ligure** (1/9/2026 sera, icona `fab fa-facebook`, tipo «Pagina Facebook», JSON-LD `SocialMediaPosting`), fra Liguria24 e Genova Quotidiana. Dieci voci. Footer delle 7 pagine a v1.3.25.
- **Previous Version**: v1.3.24 - `dicono-di-noi.html`: rassegna completata con **cinque testate in più** (in ordine cronologico, il più recente in testa): Il Secolo XIX edizione Genova (2/9, Meoli, taglio «pochi dipendenti»), Genova24 (2/9), Teleradiopace (2/9, icona `fa-tv`, tipo «Emittente locale»), Levante News e Liguria24 (1/9, stessa notizia sulla rete). Nove voci in tutto, JSON-LD `hasPart` allineato, meta description con tutte le testate. Footer delle 7 pagine a v1.3.24.
